...
describe("DetailModal.vue", () => {
  ...
  it("test DetailModal-2 overlay close-detail-modal", () => {
    expect(wrapper.emitted("close-detail-modal")).toBeUndefined();
    wrapper.find("#overlay").trigger("click");
    expect(wrapper.emitted("close-detail-modal")).toBeTruthy();
  });
  it("test DetailModal-3 content prevent-close-modal", () => {
  ...
  it("test DetailModal-18 button cancel close-detail-modal", () => {
    expect(
      wrapper
        .findAll("button")
        .at(0)
  ...
  it("test DetailModal-20 button save changeItem", async () => {
    ...
    wrapper.setProps({ passStoryName: "ストーリー3" });
    wrapper.setProps({ passStoryStatus: 3 });
    wrapper.setProps({ passStoryContents: "内容3" });
    wrapper
      .findAll("button")
      .at(1)
      .trigger("click");
  ...
  it("test DetailModal-20 button changeItem no-name", () => {
    window.alert = jest.fn();
    wrapper.setProps({ passStoryName: "" });
    wrapper.setProps({ passStoryStatus: 3 });
    wrapper.setProps({ passStoryContents: "内容3" });
    wrapper
      .findAll("button")
  ...
  it("test DetailModal-22 button remove removeItem", async () => {
    ...
    wrapper.setProps({ passStoryID:3 });
